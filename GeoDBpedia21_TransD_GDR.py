...
def main():
    ...
    eval_b_size = 32  # 测评valid test 时batch size
    
    save_time_freq = 8  # 多久进行一次测试并保存最佳模型
    require_improvement = save_time_freq*5  # 多久topk不增大则
    
#     validation_freq = 50  # 多少轮进行在验证集进行一次测试 同时保存最佳模型
